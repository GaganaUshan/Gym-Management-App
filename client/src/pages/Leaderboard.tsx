...
export default function Leaderboard() {
    ...
    const myRank = data.findIndex(d => d.userId === (user as any)?.id) + 1;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <h1
                    ...
                    style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-primary)', margin: 0, display: 'flex', alignItems: 'center', gap: 10 }}
                >
                            ...
                            <ResponsiveContainer width="100%" height={180}>
                                <BarChart data={data} barSize={28} margin={{ top: 4, right: 0, left: -20, bottom: 0 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        formatter={(val: number | undefined) => [`${val ?? 0} pts`, '']}
                                        labelFormatter={() => ''}
                                    />
                                    <Bar dataKey="score" radius={[6, 6, 0, 0]}>
                                        {data.map((entry, index) => (
                                            <Cell
                                                key={entry.userId}
                                                fill={entry.userId === (user as any)?.id
                                                    ? 'url(#myGrad)'
                                                    : index < 3 ? BAR_COLORS[index] : DEFAULT_BAR}
